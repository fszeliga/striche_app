...
import org.szeliga.taskerwidget_plugin.test.BundleTestHelper;
...
public final class EditActivityTest extends ActivityInstrumentationTestCase2<EditActivity>
{
    ...
    {
        assertEquals("Foo", EditActivity.generateBlurb(mTargetContext, "Foo")); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    private void assertActivityResultAutoSync(final String message) throws Throwable
    {
        ...
        final Runnable runnable = new Runnable()
        {
            public void run()
            {
                ...
                assertEquals(EditActivity.generateBlurb(mTargetContext, message), extras.getString(com.twofortyfouram.locale.Intent.EXTRA_STRING_BLURB));
     ...
     * 
     * @param message Hint to assert equals the EditText.
     */
